...
- `src/audio.js`：按用户手势解锁的低频反冲、方向命中与完成和弦。
...
`GameObject` 统一维护位置、速度、旋转与 DOM 同步；`Player` 增加朝向和反向冲量；`RecoilBloom` 管理弹丸、活动粒子、停驻飞溅与四边集合。每帧只更新高频对象，不触发框架重渲染。页面不可见时暂停计算，重新可见后继续。音频由原生 Web Audio 按首次触摸解锁并限频，不依赖外部音频素材。
